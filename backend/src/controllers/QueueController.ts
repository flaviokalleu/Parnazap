...
export const store = async (req: Request, res: Response): Promise<Response> => {
  const { name, color, greetingMessage, outOfHoursMessage, schedules, orderQueue,tempoRoteador, 
	ativarRoteador,integrationId, promptId } =
    req.body;
...
export const update = async (
  ...
  const { queueId } = req.params;
  const { companyId } = req.user;
  const { name, color, greetingMessage, outOfHoursMessage, schedules, orderQueue, integrationId, promptId, tempoRoteador } =
    req.body;
  ...
  const queue = await UpdateQueueService(queueId, {
    ...
    integrationId: integrationId === "" ? null : integrationId,
    promptId: promptId === "" ? null : promptId,
    tempoRoteador // Adicione esta linha
  }, companyId);
